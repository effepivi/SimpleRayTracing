...
#SBATCH --job-name=RT-pthreads       # Job name
...
do
    echo Run ./main-pthreads with $thread_number threads.
    
    /usr/bin/time --format='%e' ./main-pthreads --size $width $height --jpeg pthreads-$thread_number-${width}x$height.jpg --threads $thread_number 2> temp

    RUNTIME=`cat temp`

    echo ${CPU_MODEL[1]},Pthread,$thread_number,1,$COMPILER,${width}x$height,$RUNTIME >> timing.csv
    rm temp
