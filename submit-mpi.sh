...
do
    ...
    do
        ...
        echo "/usr/bin/time --format='%e' mpirun  ./bin-release-gcc/main-mpi --size $width $height --jpeg mpi-$NODES-$process_number-${width}x$height.jpg 2> temp-mpi-$NODES-$process_number" >> submit-MPI-$NODES-$process_number.sh
